"""JS/TS extractor runner — callable from the unified entry point."""
...
def run_jsts_extractor(target: dict, workspace_root: Path) -> dict | None:
    """Run the JS/TS extractor for a single target. Returns graph dict."""
    ...
    # Write output
    output_name = target.get("output", f"jsts_{config.project.name}_graph.json")
